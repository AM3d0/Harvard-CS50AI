...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # Give back a set of all neighbors
        neighbors = set()
        ...
        # iterate through all sentences in KB and check if there is a subset of an another set
        change = True
        while change:
            change = False
            # check every sentence, if there are new information about mines/safes. If so, then update
            for sentence in self.knowledge:
            ...
            # list of all new derived sentences in current iteration
            new_sentences = []
            # iterating through the sentences in knowlege and comparing them with each other
            for i in range(len(self.knowledge)):
                for j in range(len(self.knowledge)):
                    ...
                    if sentence1.cells == sentence2.cells:
                        continue
                    elif len(sentence1.cells) == 0 or len(sentence2.cells) == 0:
                        continue
                    # if a sentence is a subset of another one, then create a new derived sentence
                    elif sentence1.cells.issubset(sentence2.cells):
                        new_cells = sentence2.cells - sentence1.cells
                        new_count = sentence2.count - sentence1.count
                        new_sentence = Sentence(new_cells, new_count)

                       # check, if new sentence contains any information of new mines or of new safe cells
                        ...
                        # adding new_sentence to the list if its not already added
                        if new_sentence not in self.knowledge and new_sentence not in new_sentences:
                            new_sentences.append(new_sentence)
                            # since we added a new sentence, we need to start over again
                            change = True
            ...
            # before starting a new iteration, we want to add the list of new sentences to the KB
